...
int main (void) {
	...
	*mu_acr = mu_acr_val;
	...
	uint32_t * mu_arr0 = (uint32_t *) 0x41480010;
	uint32_t * mu_arr1 = (uint32_t *) 0x41480014;
	uint32_t mu_arr0_val = 0xDEADDEAD;
	uint32_t mu_arr1_val = 0xDEADDEAD;
	uint32_t * mu_asr = (uint32_t *) 0x41480020;
	...
	*mu_atr3 = 0x0;
	...
	/* BOARD_INITPINS_FTDI_M40_UART0_TX_PIN_FUNCTION_ID */
	while(!((*mu_asr)&(1<<23))){
	} 
	*mu_atr0 = 0x05060401;
	while(!((*mu_asr)&(1<<22))){
	...
	*mu_atr1 = 0x20116;
	...
	mu_arr0_val = 0xDEADDEAD;
	mu_arr0_val = *mu_arr0;		// should read 0x10201
	while(!((*mu_asr)&(1<<26))){
	}
	mu_arr1_val = *mu_arr1;		// should read 0xfbc5200
